...
- **Pre-commit**: Before every `git commit`, run `pre-commit run --all-files` on the staged files to auto-fix formatting, trailing whitespace, end-of-file issues, and catch lint errors. Re-stage any files modified by the hooks, then commit. This avoids failed commits from hook violations.
